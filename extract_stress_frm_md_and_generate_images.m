%% This script extracts stress data from the LAMMPS simulation files and generates image data for modeling cGAN. The LAMMPS input/data files can be generated from the script "generate_lammps_input_data_files.m" in this repository.
 		...
 		directory_name = ['',num2str(vac_concen),'_',num2str(x_cen),'_',num2str(y_cen),'_',num2str(x_2_cen),'_',num2str(y_2_cen)];
            ...
            oldFolder = cd(directory_name); % go to the newly created folder while saving the current location in OldFolder; SHOULD BE CHANGED FOR DIFFRENT WIDTHS
                    ...
                    if exist(dump_file_name)
                        ...
                        [data,count] = fscanf(fileID, '%d %f %f %f %f %f %*f ',[6,inf]);

                        data = sortrows(data',1); % get data in columns

                        vol_per_atom = box_vol/size(data,1);

                        corrected_stress = data(:,2:4).*((10^(-4)/vol_per_atom)); % atomis stress in GPa

                        max_stress = max(corrected_stress);
                        average_stress = mean(corrected_stress);
                        
                        fclose(fileID);

                        %% 
                        cd(oldFolder) % come back to OldFolder
                        cd('stress_distibution_yy');

                        %%

                        figure
                        h = scatter(ini_coords(:,2),ini_coords(:,3),3,corrected_stress(:,2),'fill');
                        colormap('jet') 
                        caxis([-1 5])
                        %colormap('hsv') 
                        axis equal  
